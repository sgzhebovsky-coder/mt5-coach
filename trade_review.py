...
def render_sections_html(sections):
    rows = []
    for sec in sections:
        icon_bg, icon_fg = _ICON_COLORS.get(sec["icon_kind"], _ICON_COLORS["neutral"])
        border = "border: 0.5px solid var(--color-border-secondary);" if sec["icon_kind"] == "neutral" else ""
        if sec["badge"]:
            badge_bg, badge_fg = _BADGE_COLORS.get(sec["badge_kind"], _BADGE_COLORS["success"])
            badge_html = (
                f'<div style="font-size:12px;font-weight:500;padding:4px 8px;border-radius:8px;'
                f'background:{badge_bg};color:{badge_fg};white-space:nowrap;">{sec["badge"]}</div>'
            )
        else:
            badge_html = ""
        detail_html = "".join(f'<div style="margin-top:4px;">{line}</div>' for line in sec["lines"])
        rows.append(f'''
<div style="padding:10px 12px;background:var(--color-background-secondary);border-radius:8px;margin-bottom:8px;">
  <div style="display:flex;align-items:center;gap:12px;">
    <div style="width:32px;height:32px;border-radius:50%;background:{icon_bg};{border}display:flex;align-items:center;justify-content:center;flex-shrink:0;">
      <i class="ti {sec['icon']}" style="font-size:16px;color:{icon_fg};" aria-hidden="true"></i>
    </div>
    <div style="flex:1;font-size:14px;font-weight:500;">{sec['title']}</div>
    {badge_html}
  </div>
  <div style="font-size:13px;color:var(--color-text-secondary);padding-left:44px;line-height:1.5;">{detail_html}</div>
</div>''')
    return "".join(rows)
...
def build_10_trade_review_html(sections, headline, n, has_chart):
    """HTML version of the 10-trade review: same content as the plain-text one, laid
    out as a card with a chart and 5 colour-coded sections."""
    chart_html = (
        '<img src="cid:equity_chart" alt="Balance curve" style="width:100%;border-radius:8px;margin-bottom:16px;display:block;" />'
        if has_chart else ""
    )
    return f'''
<div style="background:var(--color-background-secondary);border-radius:12px;padding:1.25rem;font-family:sans-serif;">
  <div style="font-size:13px;color:var(--color-text-secondary);margin-bottom:12px;">Trading coach &middot; 10-trade review &middot; trades {n-9}-{n}</div>
  <div style="background:var(--color-background-primary);border:0.5px solid var(--color-border-tertiary);border-radius:12px;padding:1.25rem;">
    <div style="display:flex;align-items:center;gap:10px;background:var(--color-background-success);padding:10px 14px;border-radius:8px;margin-bottom:16px;">
      <span style="font-size:14px;font-weight:500;color:var(--color-text-success);">{headline}</span>
    </div>
    {chart_html}
...
async def main():
    ...
    for deal in closing_deals:
        ...
        if n % 10 == 0:
            ...
            html_body = build_10_trade_review_html(sections, headline, n, has_chart=bool(chart_path))
