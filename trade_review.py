...
async def enrich_trade(connection, deal):
    ...
    try:
        orders = await connection.get_history_orders_by_position(position_id=info["position_id"])
...
def build_single_email(trade, n):
    ...
    lines = [
        ...
        f"Лот: {trade['volume']}",
        f"Цена входа: {trade['open_price']}   Цена выхода: {trade['close_price']}",
...
async def main():
    ...
    for deal in closing_deals:
        trade = await enrich_trade(connection, deal)
